	...
	public static int solution(int[] a) {
		...
		for (int i = 1; i < counts.length; i++) {
			...
			for (int j = 0; j < r.length; j++) {
				// negative value means we haven't reached this value, so we have to spend 1
				// current if we can
				if (r[j] >= 0) {
					r[j] = counts[i];
				...
				// the value in r[j] then means how many of the current values are left when we
				// reached the value j
			}
		}
		int result = sum;
		// don't have to traverse all the arrays, since i - the sum of elements. if it's
		// reachable then (sum - i) - reachable as well.
		// so if the value is reachable then the diff is abs(i - (sum - i)), which is
		// the same as abs(sum - 2 * i)
